...
finalmod_both_1 = lm(formula = log(overall_survival_months) ~ age_at_diagnosis + 
    chemotherapy + hormone_therapy + tumor_stage + death_from_cancer + age_at_diagnosis:hormone_therapy + 
    hormone_therapy:tumor_stage, data = cancer_new)
...
The final regression model includes the following key variables: age, chemotherapy, hormone therapy, and tumor stage (specifically stages 2 and 3). It also accounts for the outcome of death from cancer (death from cancer and death from other causes) and incorporates interaction terms between chemotherapy and tumor stage (for stages 2 and 3), as well as hormone therapy and tumor stage (for stages 2 and 3).

The results from the three model selection methods (bidirectional selection, backward selection, and forward selection) consistently provided the same set of covariates, and the covariates of the final regression model  are statistically significant on the 5% significance level. This consistency across methods enhances the credibility of the findings, indicating that the observed relationships are relatively reliable and not due to random chance.

In the model selection, we discarded radiotherapy, indicating that it doesn’t provide a significant influence on the survival period of patients. For chemotherapy, the interaction terms between chemotherapy with tumor stage 2 and chemotherapy with tumor stage 3 provide insight into the relationship between tumor stage and chemotherapy. In tumor stage 1, the slope of -62.7677 shows the most negative correlation with survival. This negative effect diminishes in stage 2 with a slope of −25.9233, and further decreases in stage 3 with a slope of −11.8983. This indicates that while chemotherapy consistently reduces survival periods, its negative effects on survival are lower at higher tumor stages. 

For hormone therapy, it initially reduces the survival period by 25.9323 at tumor stage 1, but its effect decreases to a reduction of only 5.206 units in stage 2, and it even leads to an increase in survival by 26.624 units in stage 3. From tumor stage 1 to stage 3, the impact of hormone therapy changes from harmful to beneficial, ultimately improving survival chances. This highlights the importance of carefully considering the use of hormone therapy based on tumor stage.
...
To address our initial research question, our model suggests that chemotherapy and hormone therapy have varying influences on survival time before death depending on the initial tumor stage while radiotherapy does not have a significant association with survival time before death. In general, our model suggests that chemotherapy has a negative association with survival time before death regardless of tumor stage level while hormone therapy has a negative association for patients with an initial tumor stage of 1 and 2 while it has a positive association when the patient’s original tumor stage was 3. Our model does not align with prior research as prior research suggests that chemotherapy is generally effective and increases the survival time before death. Similarly prior research suggests that hormone therapy and radiotherapy are generally effective at treating patients with IDC. However, our model aligns with prior research suggesting chemotherapy is most effective for later stages of cancer (Invasive Ductal Carcinoma, 2024).

Our model likely differs from prior research due to some limitations in our model. One limitation of our model is our variable selection method. We used forward selection to obtain our model which is sensitive to the order of variables included and is unable to remove or add back variables. Furthermore, forward selection selects one variable at a time which could lead to overlooking important interaction and could also include variables that only marginally improve the fit of the model. Although we also used backwards selection to verify the validity of our model, backwards selection also comes with similar limitations as well. Moreover, our results may have skewed results as we use tumor stage 1 as our baseline since we did not have enough data points for tumor stage 0. Overall,  the limitations of our model are reflected by the relatively low adjusted R2 of 0.3713, which indicates our model only accounts for around 37.13% of the variability in survival time before death for IDC patients. 
